...
/// A `Result` alias where the `Err` case is `reqwestplus::Error`.
...
impl Error {
    ...
    /// let response = reqwestplus::get("http://site.with.redirect.loop").await;
...
impl fmt::Debug for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let mut builder = f.debug_struct("reqwestplus::Error");
...
mod tests {
    ...
    fn roundtrip_io_error() {
        ...
        // Convert reqwestplus::Error into an io::Error...
        let io = orig.into_io();
        // Convert that io::Error back into a reqwestplus::Error...
