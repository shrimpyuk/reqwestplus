...
impl Response {
    ...
    /// let resp = reqwestplus::blocking::get("http://httpbin.org/get")?;
    ...
    ///
    /// ```rust
    /// use reqwestplus::blocking::Client;
    ...
    /// ```rust
    /// # fn run() -> Result<(), Box<std::error::Error>> {
    /// let resp = reqwestplus::blocking::get("http://httpbin.org/redirect/1")?;
    ...
    ///
    /// # fn run() -> Result<(), Error> {
    /// let json: Ip = reqwestplus::blocking::get("http://httpbin.org/ip")?.json()?;
    /// # Ok(())
    /// # }
    /// #
    /// # fn main() { }
    ...
    /// let bytes = reqwestplus::blocking::get("http://httpbin.org/ip")?.bytes()?;
    ///
    /// println!("bytes: {:?}", bytes);
    /// # Ok(())
    /// # }
    ...
    /// # extern crate reqwestplus;
    /// # fn run() -> Result<(), Box<std::error::Error>> {
    /// let content = reqwestplus::blocking::get("http://httpbin.org/range/26")?.text()?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// # extern crate reqwestplus;
    /// # fn run() -> Result<(), Box<std::error::Error>> {
    /// let content = reqwestplus::blocking::get("http://httpbin.org/range/26")?
    ...
    /// ```rust
    /// # fn run() -> Result<(), Box<std::error::Error>> {
    /// let mut resp = reqwestplus::blocking::get("http://httpbin.org/range/5")?;
    ...
    /// # extern crate reqwestplus;
    /// # fn run() -> Result<(), Box<std::error::Error>> {
    /// let res = reqwestplus::blocking::get("http://httpbin.org/status/400")?
    ///     .error_for_status();
    /// if let Err(err) = res {
